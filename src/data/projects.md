...
* **[Workflow n8n Sentiment Analysis](https://github.com/Lucas-Pontes-Soares/n8n-sentiment-analysis-gemini)** | Ago 2026 -- Set 2026  
Workflow n8n para análise de sentimentos de feedbacks dos clientes sobre produtos, via IA (Gemini).
    * Arquitetei workflow assíncrono e escalável no n8n para automação de triagem e análise de sentimentos de feedbacks de clientes;
    * Integrei a API do Google Gemini utilizando engenharia de prompt (Prompt Engineering) com saída estritamente formatada em JSON;
    * Desenvolvi rotina automática de geração sugestão de respostas para os clientes, analisado através do sentimento e risco;
    * Implementei lógica de controle de fluxo com tratamento de idempotência, apenas registros não processados sejam analisados;
    * Estruturei pipeline de dados integrando o Google Sheets como banco de dados leve via autenticação OAuth2;
    * Apliquei técnicas de rate limiting com nós de espera (Wait) para respeitar os limites operacionais da API do Gemini.

    `Tecnologias`: n8n, Gemini, IA, JSON, Workflow
...
Projeto integrador da faculdade, uma plataforma web interativa que centraliza o ensino para fortalecer o vínculo professor-aluno.
    * Desenvolvi a IA via Gemini com Prompt Engineering e RAG, os professores anexavam arquivos (slides) das aulas para contexto;
    * Construi sugestão de questões via IA e Skills para saídas JSON específicas de cada tipo (múltipla escolha, pares, lacuna);
    * Desenvolvi o Chatbot com RAG e streaming para os alunos tirarem dúvidas sobre as aulas de forma personalizada;
    * Implementei API REST em Node.js com banco relacional PostgreSQL e autenticação JWT para acesso seguro;
    * Desenvolvi front-end responsivo em React.js com Tailwind CSS e Shadcn/ui focado na usabilidade;
    * Administrei e realizei o deploy e hospedagem via Heroku.
