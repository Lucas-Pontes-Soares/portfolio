...
\noindent {\bfseries Workflow n8n Sentiment Analysis} \textbar{}  \href{https://github.com/Lucas-Pontes-Soares/n8n-sentiment-analysis-gemini}{github.com/Lucas-Pontes-Soares/n8n-sentiment-analysis-gemini} \hfill\ Ago 2026 -- Set 2026 \\
Workflow n8n para análise de sentimentos de feedbacks dos clientes sobre produtos, via IA (Gemini).
\begin{itemize}
    \item Arquitetei workflow assíncrono e escalável no n8n para automação de triagem e análise de sentimentos de feedbacks de clientes;
    \item Integrei a API do Google Gemini utilizando engenharia de prompt (Prompt Engineering) com saída estritamente formatada em JSON;
    \item Desenvolvi rotina automática de geração sugestão de respostas para os clientes, analisado através do sentimento e risco;
    \item Implementei lógica de controle de fluxo com tratamento de idempotência, apenas registros não processados sejam analisados;
    \item Estruturei pipeline de dados integrando o Google Sheets como banco de dados leve via autenticação OAuth2;
    \item Apliquei técnicas de rate limiting com nós de espera (Wait) para respeitar os limites operacionais da API do Gemini.
\end{itemize}
\noindent {\bfseries Tecnologias:} n8n, Gemini, IA, JSON, Workflow
...
\begin{itemize}
    \item Gerenciei políticas e permissões de acesso, garantindo segurança no IAM;
    \item Projetei diagramas de infraestrutura, modelando o Design System através Draw.io e excalidraw;
    \item Implementei computação escalável e rotinas orientadas a eventos com Amazon EC2 e Lambda;
    \item Estruturei persistência de dados em ambientes SQL e NoSQL gerenciados via RDS, DynamoDB e S3;
    \item Orquestrei integração assíncrona entre serviços utilizando mensageria e notificações com SQS e SNS;
    \item Consolidei ecossistema de orquestração de contêineres, criando fluxo de CI/CD utilizando Docker e EKS.
\end{itemize}
...
\end{itemize}
\noindent {\bfseries Tecnologias:} Node.js, PostgreSQL, RabbitMQ, Grafana, Jaeger, Pulumi, AWS, Git, GitHub, Drizzle, Docker, CI/CD, GitHub Actions
...
\begin{itemize}
    \item Desenvolvi a IA via Gemini com Prompt Engineering e RAG, os professores anexavam arquivos (slides) das aulas para contexto;
    \item Construi sugestão de questões via IA e Skills para saídas JSON específicas de cada tipo (múltipla escolha, pares, lacuna);
    \item Desenvolvi o Chatbot com RAG e streaming para os alunos tirarem dúvidas sobre as aulas de forma personalizada;
    \item Implementei API REST em Node.js com banco relacional PostgreSQL e autenticação JWT para acesso seguro;
    \item Desenvolvi front-end responsivo em React.js com Tailwind CSS e Shadcn/ui focado na usabilidade;
    \item Administrei e realizei o deploy e hospedagem via Heroku.
\end{itemize}
\noindent {\bfseries Tecnologias:} IA, Node.js, React.js, PostgreSQL, RAG, JWT, Heroku, Git, tailwind-css, shadcn-ui.
...
\end{itemize}
\noindent {\bfseries Tecnologias:} Docker, build, HTML, CSS, CI/CD, GitHub, Git, GitHub Actions, GitHub Pages.
